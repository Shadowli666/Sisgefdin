...
function calcularPrecios(e) {
    ...
    if (e.which == 13) {
        ...
        if (cant > 0 && cant != '') {
            const id = $('#id').val();
            registrarDetalle(e, id, cant, precio, totalbs);
            $('#servicio').focus();
        } else {
            $('#cantidad').focus();
            return false;
...
if (document.getElementById("CierreCaja")) {
    const action = "carrChart";
    $.ajax({
        url: 'chart.php',
        type: 'POST',
        async: true,
        ...
        success: function (response) {
            if (response != 0) {
				...
				var dolares = [];
                for (var i = 0; i < data.length; i++) {
                    metodo_pago.push(data[i]['metodo_pago']);
                    bolivares.push(data[i]['totalbs']);
					
                }
                var ctx = document.getElementById("CierreCaja");
                var myPChart = new Chart(ctx, {
                    type: 'pie',
					label: 'modalidad',
                    data: {
                        labels: metodo_pago,
...
if (document.getElementById("CierreCajaDolares")) {
    const action = "caChart";
    $.ajax({
        url: 'chart.php',
        type: 'POST',
        async: true,
        ...
        success: function (response) {
            if (response != 0) {
                ...
                for (var i = 0; i < data.length; i++) {
                    metodo_pago.push(data[i]['metodo_pago']);
					dolares.push(data[i]['total']);
                }
                var ctx = document.getElementById("CierreCajaDolares");
                var myPChart = new Chart(ctx, {
                    type: 'pie',
                    data: {
                        labels: metodo_pago,
...
function editarCliente(id) {
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            $('#segun_nombre').val(datos.segun_nombre);
            $("#apellido").val(datos.apellido);
            $('#segun_apellido').val(datos.segun_apellido);
...
function editarNiños(id) {
    const action = "editarNiños";
    $.ajax({
        url: 'ajax.php',
        type: 'GET',
        async: true,
        ...
        success: function (response) {
            ...
            $("#fecha_nac").val(datos.fecha_nac);
            $("#cedula").val(datos.cedula);
            $("#id").val(datos.idninos);
            $("#representante").val(datos.representante);
            $('#telefono').val(datos.telefono);
            $('#direccion').val(datos.direccion);
            $("#diagnostico_medico").val(datos.diagnostico_medico);
...
function editarVacunacion(id) {
    const action = "editarVacunacion";
    $.ajax({
        url: 'ajax.php',
        type: 'GET',
        async: true,
        data: {
            editarVacunacion: action,
            id: id
        },
        success: function (response) {
            const datos = JSON.parse(response);
