...
if (isset($_GET['q'])) {
    ...
    $servicios = mysqli_query($conexion, "SELECT * FROM servicios INNER JOIN tasa WHERE servicios.idservicios  LIKE '%" . $nombre . "%' OR descripcion LIKE '%" . $nombre . "%'");
    ...
    while ($row = mysqli_fetch_assoc($servicios)) {
        ...
        $data['label'] = $row['idservicios'] . ' - ' .$row['descripcion'];
        $data['value'] = $row['descripcion'];
        $data['precio'] = $row['precio'];
        $data['existencia'] = $row['existencia'];
    ...
    $datos = array();
    $nombre = $_GET['con'];
    $producto = mysqli_query($conexion, "SELECT * FROM consulta WHERE idconsulta LIKE '%" . $nombre . "%' OR descripcion LIKE '%" . $nombre . "%'");
    while ($row = mysqli_fetch_assoc($producto)) {
        $data['id'] = $row['idconsulta'];
        $data['label'] = $row['idconsulta'] . ' - ' .$row['descripcion'];
        $data['value'] = $row['descripcion'];
        $data['precio'] = $row['precio'];
        array_push($datos, $data);
    }
    ...
    exit;
}else if(isset($_GET['editarNiños'])){
    $idninos = $_GET['id'];
    $sql = mysqli_query($conexion, "SELECT * FROM ninos WHERE idninos = $idninos");
    $data = mysqli_fetch_array($sql);
    echo json_encode($data);
    exit;
}else if(isset($_GET['editarVacunacion'])){
    $idvacuna = $_GET['id'];
    $sql = mysqli_query($conexion, "SELECT * FROM vacunacion WHERE idvacuna = $idvacuna");
    $data = mysqli_fetch_array($sql);
    echo json_encode($data);
    exit;
    ...
    echo json_encode($data);
    exit;
} else if (isset($_GET['editarServicio'])) {
    $id = $_GET['id'];
    $sql = mysqli_query($conexion, "SELECT * FROM servicios WHERE id = $id");
    $data = mysqli_fetch_array($sql);
    echo json_encode($data);
    exit;
}else if (isset($_GET['editarPago'])) {
    $id_pago = $_GET['id'];
    $sql = mysqli_query($conexion, "SELECT * FROM detalle_pago WHERE id_pago = $id_pago");
    $data = mysqli_fetch_array($sql);
    echo json_encode($data);
    exit;
}
