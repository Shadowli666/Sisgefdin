    ...
    $("#servicio").autocomplete({
        ...
        select: function (event, ui) {
            ...
            $.ajax({
                ...
                data: {
                    obtenerMedicos: true,
                    especialidad: ui.item.especialidad
                },
                success: function (data) {
                    console.log(data)
                    var list = $("#doctor")[0]
                    ...
                    option.value = "";
                    option.label = "Seleccione un Especialista";
...
function calcularPrecio(e) {
    ...
    if (e.which == 13) {
        ...
        if (!doctor) {
            // Mostrar SweetAlert si el campo doctor está vacío
            Swal.fire({
                position: 'top-end',
                icon: 'error',
                title: 'Por favor, Ingresa el Nombre del Especialista',
                showConfirmButton: false,
                timer: 2000
            })
        ...
        if (cant > 0 && cant != '' ) {
            ...
            registrarDetalle(e, id, especialidad, doctor, cant, precio, totalbs, cotiza);
            $('#servicio').focus();
        } else {
            $('#cantidad').focus();
            return false;
...
function registrarDetalle(_e, id, especialidad, doctor, cant, precio, preciobs, taza) {
...
function calcular() {
    ...
    filas.forEach(function (e) {
        ...
        var importeBs = parseFloat(columnas[8].textContent);
        total += importe;
        totalBs += importeBs
    });
...
function crearHema(e){
    ...
    $.ajax({
        ...
        data: {
            ...
            hematrocitos:e.target.hematrocitos.value,
